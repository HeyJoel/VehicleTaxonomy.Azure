...
namespace VehicleTaxonomy.Domain.Tests.Shared.FileSources;
...
public class EmbeddedResourceFileSourceTests
{
    ...
    {
        var assembly = this.GetType().Assembly;
        var instance = new EmbeddedResourceFileSource(assembly, "VehicleTaxonomy.Domain.Tests.Tests.Shared.FileSources..Test-_Resouce!Directory()[]", ".Test re.source-!.txt");
        await using var stream = await instance.OpenReadStreamAsync();
