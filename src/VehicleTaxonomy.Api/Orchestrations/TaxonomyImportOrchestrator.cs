...
namespace VehicleTaxonomy.Api.Orchestrations;
...
public class TaxonomyImportOrchestrator
{
        ...
        )
    {
        ...
        var readBlobClient = container.GetBlobClient(fileName);

        await using var file = await readBlobClient.OpenReadAsync(cancellationToken: cancellationToken);
        ...
        async Task FileWriter(CsvFileSplitterResult result)
        {
            ...
            var writeBlobClient = container.GetBlobClient(batchFileName);
            await writeBlobClient.UploadAsync(result.File, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await using var file = await blobClient.OpenReadAsync(cancellationToken: cancellationToken);
