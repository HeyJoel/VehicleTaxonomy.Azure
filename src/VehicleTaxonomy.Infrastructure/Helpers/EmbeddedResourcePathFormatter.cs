...
public static class EmbeddedResourcePathFormatter
{
    ...
    private const char EmbeddedDirectoryDelimiter = '.';
    ...
    public static string? ConvertFromVirtualDirectory(string? path)
    {
        ...
        {
            result = path.Remove(0, VirtualPathPrefix.Length);
        }
        ...
        var filteredCharacters = result
            ...
            .Select(c => c == EmbeddedDirectoryDelimiter ? InvalidCharacterReplacement : c)
            // replace file path delimiters with the embedded resource equivalent '.'
            .Select(c => _pathDelimiters.Contains(c) ? EmbeddedDirectoryDelimiter : c)
            // replace any other invalid characters
            .Select(c => _invalidChars.Contains(c) ? InvalidCharacterReplacement : c);

        result = string
            .Concat(filteredCharacters)
            .Trim(EmbeddedDirectoryDelimiter);
